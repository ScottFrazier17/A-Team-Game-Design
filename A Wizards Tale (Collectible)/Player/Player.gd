...
const TAN_COUNT = 0.4                       #Constant of maximum "time" before the tangential velocity dies down to 0 after releasing grapple WHILE in IN_AIR state
var tangential_countdown = TAN_COUNT        #Current "time" remaining until tangential velocity dies down to 0
...
func _physics_process(delta):
  ...
  match state:                                     #match statement used for state transitioning
    ...
    PlayerState.Idle:
        ...
        state = update_state(state)                #Checks player input and changes or remains in state accordingly
    ...
    PlayerState.In_Air:
        stamina_regen(delta)                             #Regenerates stamina
        update_stamina_objects(delta)                    #Updates everything to do with stamina of the player
        if(was_in_grapple and tangential_countdown > 0): #IF the player just left grapple state, then a tangential vel is added to player WHILE in IN_AIR state
          velocity = tangential_vel
          tangential_countdown = tangential_countdown - delta
        else:                                            #IF the player was NOT in grapple state before coming to IN_AIR state, then just apply gravity acceleration and horizontal speed according to input
          velocity.y += GRAVITY*delta  
    ...
    PlayerState.Grapple:
      if(hookLocked):                                                          #if the player is attached to a hook currently
        ...
        var normal_to_grapple = Vector2(-player_to_hook_vector.y, player_to_hook_vector.x) #calculates the normal vector to vector from player to hook
        ...
        if(Input.get_axis("ui_left","ui_right")==0):                          #case when no player horizontal input in grapple state, so swinging in the direction it previously was when there was horizontal input
          ...
          if(swingingRight and !swingingLeft):
            velocity = 2.5*normal_to_grapple + 0.3*player_to_hook_vector      
        if(Input.get_axis("ui_left","ui_right") > 0):                         #case when player input to the right, so swinging right now
          swingingLeft = false
          swingingRight = true
  ...
  match curr_state: 
    ...
    PlayerState.In_Air:
      if(!is_on_floor() and jump_pressed() and has_double_jump):
        ...
        return PlayerState.Climb
      elif(hookObjects.size() > 0 and grapple_pressed()):
    ...
    PlayerState.Grapple:
      ...
      if(!(grapple_pressed() and hookObjects.size() > 0)):
